...
from app.src.schemas.base import InvestigationTask, Lead, LeadType  # Added LeadType
...
class BasicNewsRoom(NewsRoom):
    ...
    def __init__(
        self, name: str = "BasicNewsRoom", journal_dir: str | None = "app/data"
    ):
        ...
        investigator1: BasicInvestigatorOne = BasicInvestigatorOne(
            journal_dir=journal_dir
        )
        investigator2: BasicInvestigatorTwo = BasicInvestigatorTwo(
            journal_dir=journal_dir
        )
        ...
        basic_editor: BasicEditor = BasicEditor(
            name="BasicEditorForNewsRoom",
            journal_dir=journal_dir,
        ...
        self.logger.info(
            f"BasicNewsRoom '{self.name}' initialized with {len(self.investigators)} investigators and editor '{self.editor.name}'."
        )
    ...
    def process_lead_example(self, lead_content: str) -> List[InvestigationTask]:
        ...
        """
        dummy_lead: Lead = Lead(
            lead_id="dummy_lead_001", lead_type=LeadType.QUESTION, content=lead_content
        )  # Used LeadType Enum
        self.logger.info(f"Processing dummy lead: {dummy_lead.lead_id}")
        solved_tasks: List[InvestigationTask] = self.run(lead=dummy_lead)
        self.logger.info(
            f"Finished processing dummy lead: {dummy_lead.lead_id}, {len(solved_tasks)} tasks solved."
        )
        return solved_tasks
