...
from typing import List, Sequence, cast  # Added cast
...
class BasicEditor(BaseEditor):
    ...
    def __init__(
        self,
        name: str = "BasicEditor",
        journal_dir: str | None = "app/data",
        investigators: Sequence[BaseInvestigator] | None = None,
        ...
        if investigators is None:
            self.investigators: List[BaseInvestigator] = (
                [  # Keep as List for internal mutable list
            ...
            self.investigators: List[BaseInvestigator] = list(
                investigators
    ...
    def generate_plan(self, lead: Lead) -> List[InvestigationTask]:
        ...
        """
        # self.logger is now managed by NewsRoom for lead-specific logging
        ...
        self.logger.info(
            f"Editor {self.name} generated {len(tasks)} tasks for lead: {lead.lead_id}"
        )
        return tasks
    ...
    def get_investigator_for_task(self, task: InvestigationTask) -> BaseInvestigator:
        ...
        if not self.investigators:
            # self.logger is now managed by NewsRoom
        ...
        investigator: BaseInvestigator = self.investigators[
            self.investigator_idx % len(self.investigators)
        ]
        task.assigned_to = investigator.name
        # self.logger is now managed by NewsRoom
        self.logger.info(
            f"Editor {self.name} assigned task {task.task_id} to investigator {investigator.name}"
        )
        self.investigator_idx += 1
        return investigator

    # _run method is inherited from BaseEditor and uses the above methods.
    # The logger used by _run will be the one set by NewsRoom.
